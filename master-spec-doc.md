...
- **Content** — 51 roadmap items; 6 guide articles, now genuinely deepened (+45% content, §16U, every added figure verified against a primary source — ED Dear Colleague letters, the Federal Student Aid Handbook, CSS Profile's own site, studentprivacy.ed.gov), all freshness-stamped.
- **`/opportunities`** (§16R, the social-sciences close-out, and the homepage promotion, all the same day) — a real unified directory, not scholarships-only. `lib/opportunities.ts` merges `data/scholarships.ts` (**14 entries**, both Golden Door and Ron Brown resolved/listed as of §16U) with `data/major-opportunities.ts` (**24 entries** across 8 families, tagged `kind`) — **38 total**. **All 8 major families are now fully researched** — `social-sciences` was the last, closed with TASS, National Economics Challenge, Congressional Award, and We the People. `/scholarships` redirects rather than 404s. Nav says "Opportunities," not "Money" — direct user correction, do not revert. **Now advertised on the homepage** via `OpportunitiesPromo`, matching `AnalysisPromo`'s pattern exactly (own accent, live count, CTA) — see the Decisions Log entry above for the full reasoning, including why a "job" `OpportunityKind` was considered and deliberately not built.
