...
- **Content** — 51 roadmap items; **7 guide articles** (a 7th, "Community College and Transfer Pathways," added Aug 17, 2026), the original six genuinely deepened (+45% content, §16U, every added figure verified against a primary source — ED Dear Colleague letters, the Federal Student Aid Handbook, CSS Profile's own site, studentprivacy.ed.gov), all freshness-stamped.
- **`/opportunities`** (§16R, the social-sciences close-out, the homepage promotion, the experience-first reframe, and two further content-expansion passes, all Aug 17, 2026) — a real unified directory, not scholarships-only. `lib/opportunities.ts` merges `data/scholarships.ts` (**16 entries**) with `data/major-opportunities.ts` (**29 entries** across 8 families, tagged `kind`) — **45 total** as of the latest pass. **All 8 major families are researched**, though `education` remains the thinnest at 2 entries — see the Decisions Log entries above for exactly what's been tried and ruled out there. `/scholarships` redirects rather than 404s. Nav says "Opportunities," not "Money" — direct user correction, do not revert. Homepage and hero copy both lead with **"Real experience, not just free money"** — also a direct user correction, don't revert to money-first framing. **This directory is now standing ongoing work**, not a finished feature — the user explicitly asked for continued expansion; see the memory file `feedback_autonomous_build.md`.
