...
- **Aug 17, 2026 (later session) — fixed a real bug in the OpenRouter model list before it could bite.** The free-model chain in `lib/ai/openrouter.ts` listed three model ids written from training-data memory during an earlier pass — none of which exist on OpenRouter's live catalog. Every one would have 404'd, so the resume-rewrite route would have silently fallen through to Gemini on every call, invisible only because `OPENROUTER_API_KEY` was never set. Caught by fetching the raw `GET /api/v1/models` JSON directly (a WebFetch summary of the same endpoint truncated to 5 of 17 free models — not reliable for a 400+ entry list) and diffing against what was actually in the file. Replaced with 5 verified-live free models (`google/gemma-4-31b-it`, `z-ai/glm-5.2`, `openai/gpt-oss-20b`, two Nemotron sizes), documented `.env.local.example` for the key (it had no entry at all before), and added a regression test asserting the chain shape (multiple entries, `:free`-pinned, `provider/model` slugs) — the live-catalog check itself has to stay a manual re-verification, since a unit test can't call OpenRouter from CI. **Free-tier model availability on OpenRouter churns fast — re-verify against the live endpoint before trusting this list again after any real time gap, don't assume it still matches.**

- **Aug 17, 2026 (final session of the day) — `social-sciences` researched, closing the last thin major-opportunity family.** Four real, verified entries added to `src/data/major-opportunities.ts`, each checked directly against its official or near-official site rather than recalled: **Telluride Association Summer Seminar (TASS)** — fully free including travel, verified on tellurideassociation.org, but the ~2026 cycle already closed (Dec 3, 2025) so timing is stated as a pattern to expect for 2027, not a live deadline; **National Economics Challenge** — free team competition via the Council for Economic Education, confirmed the David Ricardo division exists specifically so first-timers aren't up against AP/returning students; **Congressional Award** — self-paced, non-competitive, no GPA gate; the registration fee had a real discrepancy between sources (a state affiliate council says $25, a third-party guide says $35) and is stated as a range to confirm rather than picked arbitrarily, matching the Golden Door/Ron Brown honesty pattern; **We the People: The Citizen and the Constitution** — a class-based constitutional competition where cost isn't published anywhere as a per-student fee, so the card says exactly that and points the reader to their own civics teacher rather than guessing a number. `stillResearching` removed from the family — **all 8 major families are now fully researched**, and `/opportunities` reached **37 entries total** (14 scholarships + 23 major-linked, up from 19). One accepted overlap: TASS was already listed under `humanities` from an earlier research pass with a different (also-verified) writeup — kept both rather than deduping, matching the existing precedent of RSI/NIH SIP appearing under two families when a program genuinely serves both. Verified live in a browser (search-filtered to each of the 4 new entries individually) before shipping, not just via the test suite.

- **Aug 17, 2026 (same session, continued) — design signed off as final; `/opportunities` promoted on the homepage; the first real paid-job entry added.** The user gave an explicit, unambiguous design sign-off ("everything you have done so far is good... im signing off on design after this") — the accent/backdrop/typography system is no longer provisional, see the design-direction section of CLAUDE.md, which now states this plainly rather than "not yet confirmed." Confirmed `OPENROUTER_API_KEY` is now set both locally and in Vercel by the user, so the resume-rewrite route runs on real OpenRouter rotation, not just the Gemini fallback, for the first time.
  - **`OpportunitiesPromo`** (`src/components/home/OpportunitiesPromo.tsx`) — a new homepage section, placed directly after `AnalysisPromo`, matching its exact pattern: `data-accent="lime"` (the opportunities page's own color, not the homepage's teal — same "this is a door to somewhere else" reasoning `AnalysisPromo` already established), a live entry count pulled from `allOpportunities()` so the copy can't go stale the way a hardcoded number would, three feature cells (free money / paid work / grade filtering), and a CTA to `/opportunities`. Directly answers the user's explicit ask: "make it a big feature and make sure its obvious to the public add it on main page as advertisement."
  - **Youth Conservation Corps (YCC)** added to the `natural-sciences` family as an `internship`-kind entry — verified against the U.S. Forest Service's own YCC page. The first entry on the entire list that is a genuine paycheck (federal/state minimum wage) rather than a scholarship, stipend, or competition prize, added specifically because the user asked for real summer-job content, not just money and academic placements. Its application process is honestly described as decentralized (apply through corpsnetwork.org or a local forest/park/refuge office, no single national portal) rather than invented into a cleaner single process than actually exists — same honesty pattern as Boys State's state-by-state cost variance.
  - **A broader summer-jobs category was considered and deliberately not built as a new type.** The existing `internship` kind ("real placed work, usually paid, in an actual lab or role") already covers YCC correctly; adding a fifth `OpportunityKind` for one entry would have meant touching the type union in two data files, the merge layer, and the filter UI in `OpportunityDirectory.tsx` for no real gain over just using the type that already fits. **Most city/state summer-youth-employment programs (NYC's SYEP, LA's Hire LA Youth, and dozens of others) were deliberately NOT added** — they're too numerous and too locally specific to enumerate honestly in a national list, the same reasoning already applied to Boys State's sponsorship variance. If this becomes a bigger ask later, the right shape is probably a short explainer ("most major cities run one of these, here's how to find yours") rather than trying to list every city's program by name.
  - **The Student Conservation Association's National Crews program was researched and deliberately NOT added** — its own site states the program is "currently on hiatus." Same rule as Bank of America Student Leaders on Aug 17 earlier: don't list something that isn't currently real, even when the organization and history are legitimate.
  - Full verify (typecheck, lint, 58 tests, build) plus live browser verification (scrolled to the new section, confirmed the live count and copy render) before shipping.

- **Aug 17, 2026 (immediately after) — reframed opportunities as experience-first, not money-first, per direct user correction.** The user flagged the promo screenshot: a JSX whitespace bug (same documented class as the one in CLAUDE.md's design-direction section — an expression followed by wrapping text loses its space without an explicit `{" "}`) had rendered "38VERIFIED" with no space, and separately, they explicitly rejected the "Money, and everywhere else it comes from" headline: "dont advertise it as money advertise as ec for college or experience... from job or internshps." Fixed the spacing bug, and rewrote both `OpportunitiesPromo` and `/opportunities`' own hero to "Real experience, not just free money" — internships, programs and paid work lead now, scholarships are mentioned honestly but not the headline. Also reordered the promo's three feature cells to match (experience → paid work → money, previously money-first). Verified live in-browser on both pages before shipping.

- **Aug 17, 2026 (later, user asked to keep expanding content indefinitely) — deepened the four thinnest opportunity families.** `health-medicine`, `humanities`, `business` were all sitting at 2 entries versus 4–5 elsewhere; added one real, individually-verified entry to each (plus a second to humanities via cross-listing): **HOSA – Future Health Professionals** (health-medicine; chapter-based like DECA, fee not nationally published — same "ask your advisor" honesty pattern), **National History Day** (humanities; grades 6–12, no found national entry fee, 2027 National Contest confirmed June 13–17 at the University of Maryland), **Scholastic Art & Writing Awards** cross-listed under humanities (already existed under `arts-design`; its writing categories fit humanities as directly as TASS already fits both humanities and social-sciences — same accepted-overlap precedent), **FBLA** (business; $10 national dues, state/local variable, same shape as DECA) and **Diamond Challenge** (business; free, $100k prize pool, 2027 cycle confirmed: submissions open Sept 16 2026, close Jan 14 2027, summit Apr 29–30 2027). `natural-sciences`' `verifiedOn` was also corrected to Aug 17 — it had gained YCC in an earlier commit that day without the date being bumped. New family counts: engineering-cs 4, natural-sciences 4, humanities 4, business 4, arts-design 3, health-medicine 3, social-sciences 5, **education still 2** (the one family a further search couldn't clear the verification bar for — leads were all single-university regional pre-med pipelines, not national programs). `/opportunities` reached 43 entries at this point (24→29 major-linked).

- **Aug 17, 2026 (same "keep expanding" instruction, continued) — a 7th guide article, plus two new scholarships.** Added **"Community College and Transfer Pathways"** to `guide-articles.json` — covers why the cost gap is real ($4,150/yr community college vs. $11,950/yr public four-year in-state, College Board's own Trends in College Pricing and Student Aid 2025 report), how articulation agreements and 2+2 pathways actually work (Florida's statewide AA-guarantees-admission system and the UC Transfer Admission Guarantee as two concrete, honestly-scoped examples — explicitly not claimed to generalize to every state), how to check what actually transfers before enrolling (ASSIST.org as California's own example, others told to ask their target school directly), and the one real caveat on financial aid: the Pell Grant's 12-semester/600% lifetime limit, verified against the Federal Student Aid Handbook. `guide-articles.test.ts`'s hardcoded `toHaveLength(6)` was bumped to 7 — a real test update, not a bug fix — and a matching `freshness.ts` stamp was added. Also added **National Merit Scholarship Program** (corroborated via the College Board's own PSAT/NMSQT page and consistent secondary sources, since nationalmerit.org itself returned HTTP 402 to automated fetches across multiple attempts — flagged honestly in the entry's own `cycle` text rather than silently presented as directly verified) and **Burger King Scholars** (fully verified via the official `burgerking.scholarsapply.org` portal: $1,000–$60,000, genuinely low 2.0 GPA bar for the general track, 2027–28 cycle opens Oct 15 2026 / closes Dec 15 2026 or at 30,000 applications). **Foot Locker Scholar Athletes was researched and deliberately not added** — the only page reachable returned a stale, archived 2017–18 cycle, and other sources gave conflicting current dates; same "don't guess" rule as Bank of America Student Leaders and SCA National Crews earlier the same day. Scholarships reached 16; `/opportunities` reached **45 entries total**.
  - **Standing note for future sessions:** the user explicitly asked for this to be ongoing background work ("keep expanding any piece of content that can be perfected... keep adding content"), not a one-time task. Treat `education`'s thin count and the overall opportunities/scholarships lists as always-open, never "finished," unless the user says to stop. See the memory file `feedback_autonomous_build.md` for the standing instruction to keep executing this kind of clear-direction work without pausing to check in.

- **Aug 17, 2026 (continuing the standing "keep expanding content" instruction) — an 8th guide article, the last thin opportunity family closed, and a new deadline visual.**
  - **`education` is no longer thin.** Added the **AAPT Barbara Lotze Scholarship for Future Physics Teachers**, verified directly on AAPT's own page: up to $3,000, renewable for each of four years (so up to $12,000, not a one-off), open to high school seniors already accepted into a physics teacher-prep program, U.S. citizens, December 1 deadline. It's narrow on purpose and the entry says so — physics teaching is a documented shortage field, which is exactly why the money exists and the applicant pool is small. **Every major family now has 3+ entries.**
  - **8th guide article: "After the Decisions Arrive."** A genuine gap — the guide covered applying thoroughly but stopped at the moment decisions land, which is where the money is actually decided. Covers: comparing offers by cost-of-attendance-minus-grants-only (never the headline "total aid" number, which mixes grants with loans); the federal **professional judgment** authority for aid appeals, including the detail that the aid administrator's decision legally cannot be overruled by the college president or ED; waitlist mechanics (you usually must actively claim the spot or you're dropped, and movement only happens after the reply date); deferrals; **May 1 and double-depositing**, framed as a real integrity violation that gets offers rescinded, with the one narrow recognized exception (holding a spot while on another school's waitlist); conditional admission and senior-grade collapse; and what to do if nothing worked out. Verified against the College Board's counselor-facing application-ethics guidance and federal student aid guidance on the HEA provision.
    - ⚠️ **A deliberate omission worth preserving:** the widely-quoted "68.7% of rescissions are for final grades" figure comes from a NACAC survey that stopped being collected years ago. The article describes the mechanism and explicitly states the frequency is unknown rather than citing a stale number as current, and `freshness.ts` carries a warning not to add one back.
    - **A testing article was considered and deliberately NOT written.** Article 1 already has a substantial, current "Standardized testing: a genuinely current, still-shifting picture" section covering test-optional vs test-free, the Ivy reinstatements, and the check-each-school rule. A separate article would have duplicated it and created two places to keep in sync on a fast-moving policy. Checked the existing headings first rather than assuming the gap.
  - **`DeadlineTimeline` on `/opportunities`** (`src/components/opportunities/DeadlineTimeline.tsx`) — a twelve-month track plotting scholarship deadlines on a real time axis, placed above the directory. The directory answers "what exists"; this answers "what's next," and shows the one thing a sorted list structurally hides: that deadlines **cluster** (seven of them fall between mid-September and mid-January), which is the actual argument for starting early. Reuses `DeadlineOrbit`'s exact urgency thresholds (≤14d red, ≤60d amber, else calm) so the homepage clock and this track read as one instrument rather than two palettes. **Only entries with a published `closesOn` are plotted** — undated and already-closed awards are counted in a note underneath instead of being given invented positions, same honesty rule as the data files. Verified the marker math against every deadline by hand (Gates 28d → 7.67%, APIA 150d → 41.1%, etc.), confirmed the track agrees with the directory card's day count rather than drifting from it, and confirmed at 375px that the page itself does not scroll horizontally while the track scrolls inside its own container.
  - **The JSX whitespace bug bit again and was caught by measuring, not reading** — `{cond ? "It's" : "They're"} still real` rendered as "They'restill real". Same documented class as the `38VERIFIED` bug earlier the same day; fixed with an explicit `{" "}`. Worth noting that both instances were found by extracting rendered text and asserting on it, not by looking at the JSX.

- **Aug 17, 2026 (large batch at the user's direction: saved opportunities, a goals field, two more guide articles, timeline extensions).**
  - **Saved opportunities — migration `0010`.** A star on every card plus a "★ Saved (n)" filter. Stores the same string id `lib/opportunities.ts` computes for React keys, **not** a foreign key, because the opportunities are hand-verified TypeScript rather than rows. Consequence recorded deliberately: a renamed entry can orphan a saved row, which is handled by not rendering unmatched ids — **never** by deleting the row, since a scholarship pulled for one cycle often returns. Optimistic toggle that **reverts on failure**, because a star that lies defeats the entire purpose. `unavailable` is a first-class state: if the table is missing (or there's no session, as in local dev) the control hides itself entirely rather than rendering buttons that silently do nothing. Verified that degradation live before shipping. Saved rows are deliberately NOT in `get_shared_progress` — what a student is *considering applying for*, especially status-dependent awards, is more sensitive than roadmap progress, not less.
  - **The goals box — migration `0011`.** `profiles.goals_notes`, free text, on `/stats`. Every other profile field is something we chose to ask; this is the first place a student can say what they actually want ("something with computers and healthcare", "I help run my family's shop and want that to count"). **Deliberately not parsed into tags** — the value is exactly the part that doesn't fit a taxonomy. Wired into `buildContextBlock` with an instruction that is as load-bearing as the data: anchor to what they wrote, name real conflicts, but **do not substitute a more conventional goal, and do not praise the plan just because they wrote it down**. Profile Analysis shows it back at the top and, when empty, says plainly that the page can otherwise only work from numbers, which is "the thinnest possible version of you". Not in the counselor share link, and the UI says so.
  - **Guide article 8, "After the Decisions Arrive."** Covered in the previous entry's spirit but shipped here: comparing offers by cost-of-attendance-minus-grants-only, the federal professional-judgment appeal (including that the aid administrator's decision cannot be overruled by anyone above them), waitlist opt-in mechanics, deferrals, May 1, double-depositing as a real rescission risk with its one narrow exception, and conditional admission. **Deliberately omits the widely-quoted "68.7% of rescissions are for final grades"** — that survey stopped being collected years ago; `freshness.ts` carries a warning not to add it back.
  - **Guide article 9, "How to Actually Do Research in High School."** Three routes, ranked by how few people try them: structured programmes (competitive, apply-don't-plan), **cold-emailing a professor** (the one almost nobody attempts — specificity, proximity, and targeting reachable regional universities rather than famous ones), and self-directed work using public data. Explicitly includes humanities and social science as real research, because "do research" is heard as a STEM instruction. Carries a plain warning about **paid research programmes and pay-to-publish journals**, with a stated test (legitimate opportunities cost nothing, pay you, or waive a modest fee) rather than naming operators, which would go stale and invite a dispute. Linked from `/major` under the programmes list, specifically so that list doesn't read as "these or nothing."
  - **Deepened the GPA article with dual enrollment, community college, and the UC rules** — a direct user request rooted in their own confusion. Verified against UC's own GPA page: the summer-after-9th-through-summer-after-11th window, A=4/B=3/C=2/D=1 with plus/minus ignored, 3.0 resident / 3.4 nonresident minimums, **UC-transferable community college courses earning an honors point for residents** (AP/IB only for nonresidents), the 8-point cap, the 4-point 10th-grade sub-limit, and no bonus on D/F. States plainly that a community college grade creates a **permanent college transcript** an AP score never does. **One claim was caught and corrected before shipping:** a draft said California "waives" enrollment fees for high schoolers — Ed Code 76300(f) actually *permits each district's governing board* to exempt them. Rewritten as a district-level decision, with a ⚠️ in `freshness.ts` never to restate it as a blanket statewide waiver.
  - **Timeline extended to programmes, plus a per-grade filter.** `Opportunity` gained optional `opensOn`/`closesOn`, and **only three entries got them** (YoungArts, the Wharton investment competition, Diamond Challenge) — the ones whose organisations publish hard dates. TASS was explicitly NOT given dates despite having a known pattern, because "expect it to open mid-October based on last year" is a projection and placing it on a time axis would render it as fact. Track went 7 → 10 markers. The grade filter mirrors the directory's rule: it narrows entries with structured grades and **never filters programmes out**, since hiding one because we couldn't parse "rising seniors" would misrepresent what exists.

- **Aug 18, 2026 — migrations verified live, a measured perf fix, a real content-leak bug, guide visuals, and an acquisition plan.**
  - **Migrations 0010 and 0011 confirmed applied**, not by trusting the dashboard's "Success" message but by querying the live REST API directly: `saved_opportunities` and `profiles.goals_notes` both return 200 rather than the 404/400 a missing table or column would produce. Saved opportunities and the goals box are live.
  - **Fixed the typing lag on `/opportunities`, after measuring rather than guessing.** Each keystroke cost **37–70ms** of blocking work against a 16ms frame budget, so typing visibly stuttered. Root cause was not the obvious suspect: every card was wrapped in a framer-motion `FadeIn` whose `delay` was derived from the card's index **in the filtered list**, so filtering changed the indices and every visible card re-animated on every character. Fix was a stable `key` plus the existing `.swap-in` CSS keyframe (cards animate once on arrival, stay still while typing) and `memo` on `EntryCard` with a referentially-stable `onToggleSave` — an inline `() => toggle(e.id)` would have defeated the memo entirely. Re-measured: **8–12ms**, roughly 6× faster. ⚠️ Do not reintroduce an index-derived animation delay into a filtered list; the comment in the file says so.
  - **A real content-leak bug, found while acting on user feedback.** The user called the article footer "bullshit about honesty" and asked for it gone. Inspecting it revealed something worse than tone: `FreshnessStamp` was rendering `freshness.ts`'s `checked` and `watch` fields **verbatim to students**, and those are maintainer notes — `watch` literally contained instructions to future editors like "⚠️ Do NOT add acceptance-rate numbers for RSI." Internal process was being shown to 16-year-olds on a public page. Now it renders one quiet line ("Last checked 17 August 2026…"); the fields remain in `freshness.ts` as the maintainer record they always were. **The lesson generalises: never render a maintainer-facing field to users. Add a user-facing field instead.** Similar hedging copy on `/opportunities`, `DeadlineTimeline` and the roadmap Opportunities block was trimmed to plain "confirm on the site" language.
  - **Guide articles gained a visual system** — `components/guide/GuideVisual.tsx`, four server-rendered variants (`table`, `bars`, `steps`, `example`), attached via an optional `visual` field on any section. **10 visuals so far.** The strongest is a complete **annotated cold email to a professor** in the research article, with six notes on why each part works — the thing that actually unblocks a student. Also: an AP vs dual-enrollment vs community-college comparison, UC's GPA calculation as steps, community-college vs four-year tuition as bars, a **two-aid-offers table where both letters announce $38,000 and the families pay $10,000 and $26,000**, federal Pell/loan ceilings, the activities-list before/after, and the four deadline types. Figures break out wider than the prose column on `lg`; tables scroll in their own container so the page body never does. Hard rule in the file header: a visual may only render something the article already verified — a chart makes a number look more authoritative, so an unverified one does more damage here than in a paragraph.
  - **MIT THINK Scholars added; Bank of America Student Leaders deliberately rejected for the second time, now permanently documented.** Every aggregator describes BofA Student Leaders as a paid internship for high school juniors and seniors. **Bank of America's own current FAQ requires applicants to be 18+, to hold a high school diploma earned within the last two years, to have completed 12–18 credit hours of post-secondary coursework, and to be enrolled at a post-secondary institution.** That is a college program. Listing it on secondary-source strength would have sent juniors to an application they cannot enter. A new "PROGRAMS CHECKED AND DELIBERATELY NOT LISTED" block in `major-opportunities.ts` records this along with SCA National Crews and Foot Locker, so nobody re-researches them and reaches a worse conclusion. MIT THINK was verified by extracting text from MIT's own guidelines PDF (the site is JS-rendered and returned nothing useful) — eligibility and structure confirmed there; the $1,000 figure and exact dates were NOT legible in it and are flagged as needing confirmation rather than stated. `/opportunities` is now **47 entries**.
  - **A user-acquisition playbook** was written and published as a shared artifact: four phases (people you know → your own counselor → local Fremont organisations → online last), with copy-paste scripts for each and a Reddit strategy built around posting the UC GPA content as genuinely useful material rather than a link drop. Core argument: **go local first**, because a stranger who bounces teaches you nothing while a classmate will say "I didn't know what to click", and because channels don't reset — burning r/ApplyingToCollege on a version with findable problems costs the best channel permanently. Honest gap flagged in the document itself: that subreddit's exact current rule text could not be retrieved, so it says to read the sidebar rather than quoting rules that may have changed.

- **Aug 18, 2026 (later) — visuals extended to the roadmap, a mobile table fix, and the early-grade scholarship gap closed.**
  - **The guide visual system now serves the roadmap too.** Roadmap items already used the identical `sections: [{heading?, paragraphs[]}]` shape as guide articles, so this needed a type addition and one render line rather than a second system. **6 roadmap visuals** so far, chosen where the content is genuinely comparative or procedural: the **8th-grade math placement cascade** (two tracks side by side showing that one middle-school placement decides whether calculus is reachable by senior year — probably the least-known high-stakes fact in the whole file), reach/target/safety definitions with counts, the columns an application tracker actually needs, the testing year in order, what goes in a brag sheet, and the offer-comparison method as a procedure.
  - **Comparison tables were unreadable on phones, and the fix was measured.** `min-w-[520px]` meant a two-column comparison showed exactly ONE column at 375px — the reader had to hold one side in their head while scrolling to the other, which defeats the entire point of a comparison. Min-width is now derived from column count (`96 + columns × 100`), so a two-column table lands at 296px and **fits a 375px viewport exactly** (verified: client 299 / scroll 299), while a three-column table still scrolls internally (434 / 277) rather than crushing its cells. Page-level horizontal overflow remains zero throughout.
  - **The early-grade scholarship gap, found by auditing rather than by feel.** 13 of 16 scholarships were grade-12 only, so a 9th or 10th grader using the grade filter saw almost nothing — in an app whose stated audience is grades 6–12. Closed with two awards verified on their own sites: the **Gloria Barron Prize** (ages 8–18, $10,000 to 25 winners, judged on a service project you led yourself — one of the very few things here open to middle schoolers) and **Davidson Fellows** ($25k/$50k/$100k, age-based with no minimum, explicitly excluding community service and arts categories). A 9th-grade filter now returns 33 of 49 entries rather than a senior-only wall.
  - **Two data tests caught a real error during this, and a third problem they could not catch was found by re-reading.** The freshness tests failed because the Barron Prize `cycle` text named bare months with no year — rule 2 of that file, working exactly as designed. Re-reading the entry then surfaced something worse and untested: an `opensOn: "2027-01-15"` that **the organisation never publishes anywhere** — invented while writing the entry. Removed. The retained `closesOn: 2027-04-15` is derived from the site's own "each year" phrasing and the entry says so explicitly rather than presenting it as a published 2027 calendar date. **The lesson to keep: a passing test suite does not mean the data is honest — only re-reading the source does.**

## 15. Current Build Status

*(Update after every session — this is what you paste into a new AI tool to catch it up instantly. This section has gone stale multiple times before — Aug 15, again Aug 16 — so treat any gap between this section and `git log` as a signal to re-audit, not as "nothing happened.")*

**Rewritten in full Aug 17, 2026, end of a long multi-session day, at the user's explicit request ("summarize everything that has been done and needs to be done... so the next one can pick up exactly from here") before clearing the session. Every fact below was re-verified directly against the repo/database in this pass, not recalled from earlier doc text — see the Decisions Log entries for Aug 17 for the individual verification steps. Read "What a new session should pick up" first — that's the actual handoff.**

### What's live — essentially all of V1 + V2, including everything §16K planned

- **Accounts/auth/backend** — email/password + Google OAuth, session refresh, route gating, `profiles`/`roadmap_progress`/`activities`/`chat_messages`/`share_links`/`courses` schema, all RLS-enabled, correctly-scoped account deletion. **Migrations 0001–0009 all confirmed applied** to the live database.
- **Onboarding branches by role** (built this session, §16S) — parents get the standalone-account promise on screen at the moment of choosing "parent," are never asked to identify their child, and land on `/guide`; students land on their grade's roadmap as before. `account_type` finally has a real signup path after existing unused in the schema since migration 0001.
- **Content** — 51 roadmap items (6 visuals); **9 guide articles** (10 visuals) (a 7th, "Community College and Transfer Pathways," and an 8th, "After the Decisions Arrive," both added Aug 17, 2026), the original six genuinely deepened (+45% content, §16U, every added figure verified against a primary source — ED Dear Colleague letters, the Federal Student Aid Handbook, CSS Profile's own site, studentprivacy.ed.gov), all freshness-stamped.
- **`/opportunities`** (§16R, the social-sciences close-out, the homepage promotion, the experience-first reframe, and two further content-expansion passes, all Aug 17, 2026) — a real unified directory, not scholarships-only. `lib/opportunities.ts` merges `data/scholarships.ts` (**16 entries**) with `data/major-opportunities.ts` (**31 entries** across 8 families, tagged `kind`) — **49 total** as of the latest pass. **All 8 major families are researched and none is thin any more** — `education` was the last, closed with the AAPT Barbara Lotze scholarship. A `DeadlineTimeline` above the directory plots the dated deadlines on a twelve-month axis. `/scholarships` redirects rather than 404s. Nav says "Opportunities," not "Money" — direct user correction, do not revert. Homepage and hero copy both lead with **"Real experience, not just free money"** — also a direct user correction, don't revert to money-first framing. **This directory is now standing ongoing work**, not a finished feature — the user explicitly asked for continued expansion; see the memory file `feedback_autonomous_build.md`.
- **`/major`** (§16L) — course ladders, 4-stage grade timeline, structure facts, per-family compare grid. `MajorLens` on the roadmap stays a one-line pointer on purpose.
- **`/stats`** (§16P) — grade, major, a real per-course list (not just the coarse rigor bucket), and four student-reported school-context fields, all feeding `buildContextBlock()` with a describe-don't-rank instruction. `/account` shrank to identity/settings only as part of the same move.
- **`/tools/profile-analysis`** (§16Q) — the flagship, live and promoted on the homepage. Deterministic course-vs-major matching (digit-aware, regression-tested against the "Algebra I ≠ Algebra 2" bug), AI activity rewriting under the never-inflate rule, and a dream-college section built on NCES/Common Data Set aggregate ranges rather than the ML model (see below). Plus `/tools`, `/tools/fee-waivers`, `/tools/essay-brainstorm` as their own routes.
- **AI, three surfaces, one shared cap** — Ask AI, the activities interview, and Profile Analysis's resume rewrite, all through `lib/ai/guard.ts`. Gemini fallback chain (`3.7-flash` → `3.5-flash` → `3.1-flash-lite`) for the first two; OpenRouter free-model rotation attempted first for the resume rewrite specifically, falling back to Gemini.
  - **A real uncapped spend hole was found and closed this session (§16U, migration 0009):** the resume-rewrite route checked the daily cap but never wrote a usage row, so it was the single most expensive call in the app while being effectively free to spam. Fixed; `AI_DAILY_CAP` and `AI_DISABLED=1` are now dashboard-settable kill levers, tested.
  - **A real bug in the OpenRouter model list was found and fixed the same day, later than §16U** (see the Aug 17 Decisions Log entry near the top of this doc): three model ids in `OPENROUTER_CHAIN` were invented from training-data memory and didn't exist on OpenRouter's live catalog — every one would have 404'd silently the day the key was ever set. Replaced with 5 ids verified directly against the live `/api/v1/models` endpoint, documented in `.env.local.example` (which previously had no mention of the key at all), and covered by a shape-assertion regression test (`openrouter.test.ts`).
- **Design/perf/text-rendering** (§16Q) — a real user-reported text-clipping bug (GSAP `mask: "lines"` slicing descenders off animated headings) and a real perf bug (a `scale()` inside `.aurora`'s blurred keyframes forcing full re-rasterization every frame, worse than all the WebGL combined) were both found and fixed. Canvases now pause off-screen via a shared `useSyncExternalStore` hook (`lib/useWebglGate.ts`), replacing four near-identical hand-rolled copies.
- **Mobile** — a real, documented audit (§16T): 17 routes at 375px, zero overflow, zero iOS-zoom triggers, touch targets fixed with a hit-area utility. Supersedes the earlier "three separate symptom-driven commits" state.
- **Automation** (§16S) — `scripts/ship.sh` (verify → commit → push, refuses to push on failure) is the standing ship path. **58 vitest tests**, pure logic/data integrity only. **CI is genuinely live on `origin/master`** (`.github/workflows/verify.yml`) — landed via GitHub's web editor after the local push was blocked by the PAT's missing `workflow` scope; see the update note earlier in §16S. Vercel Analytics + Speed Insights are live (cookieless, no product analytics).
- **Deployment:** `origin/master` and local fully in sync as of the last push (`30ace13`, the OpenRouter fix). Auto-deploys to **https://pathfinder-atharv.vercel.app**.

### The ML-model question — RESOLVED Aug 16, 2026, do not re-open

The trained model stays the brother's side project and never ships into PathFinder. Profile Analysis's dream-college comparison is built on published NCES/Common Data Set aggregate ranges instead — descriptive facts about a school's last admitted class, never a probability or a verdict about this student. See §16N for the framing rules and §16O for the side project's own verified build plan. Don't re-litigate either.

### What a new session should actually pick up — nothing else is queued

1. **Design is signed off as final** (Aug 17, 2026, the user's explicit words quoted in the Decisions Log entry above) — don't re-propose or treat the current system as provisional.
2. **`OPENROUTER_API_KEY` is confirmed set, both in `app/.env.local` and the Vercel dashboard** (user-confirmed same day) — the resume-rewrite route now genuinely runs OpenRouter's free-model rotation first, not just the Gemini fallback. **The paid provider upgrade (Claude Sonnet) is still a deliberately deferred, not-forgotten decision** — the user explicitly redirected an attempt to wire this in earlier the same day ("no do provider setup for multiple ai models with openrouter it must all be free for now i can upgrade later"). `ANTHROPIC_API_KEY` stays reserved and unused until the user asks for it.
3. **26 React Compiler lint warnings remain, by design** — all inside verified WebGL/animation code where the flagged imperative patterns are correct; reasoning is in `eslint.config.mjs`. Recorded debt, not urgent.
4. The `workflow`-scope PAT limitation is unresolved but no longer blocking anything (CI landed via the web-editor workaround) — fixing it (`git credential-osxkeychain erase` + a rescoped token) is a nice-to-have, not a task.

**Content research is now fully caught up** — all 8 major-opportunity families are researched (`social-sciences` was the last, closed the same day as this rewrite — see the Decisions Log entry just above Section 15), Golden Door and Ron Brown are both resolved, and the guide-article depth pass is done. No §16K item, no known-open-item from earlier handoffs, and no "next feature" from the user's backlog remains outstanding as of this rewrite.
...
**Update, same day, later session: landed anyway, via GitHub's web editor.** Rather than wait on the token scope, the user directed pasting `verify.yml`'s content directly into GitHub's browser-based file editor (CodeMirror), which commits through the web UI and isn't subject to the PAT's scopes at all. One paste attempt was silently corrupted by the editor's autocomplete (a dropped space, then a further-corrupted retry); the fix was to stop patching characters and instead select-all + retype the affected line, then verify the *entire* file byte-for-byte against `git show ci-workflow-pending:.github/workflows/verify.yml` via `javascript_tool` before committing. `.github/workflows/verify.yml` is now genuinely on `origin/master` (confirmed via `git ls-tree -r origin/master`), and the `ci-workflow-pending` branch was deleted as superseded. The token-scope fix above is still worth doing eventually, but CI is not blocked on it anymore.
