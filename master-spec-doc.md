...
- **`/opportunities`** (§16R) — a real unified directory, not scholarships-only. `lib/opportunities.ts` merges `data/scholarships.ts` (**14 entries**, both Golden Door and Ron Brown resolved/listed as of §16U) with `data/major-opportunities.ts` (**19 entries** across 8 families, tagged `kind`). **7 of 8 major families are fully researched**; only `social-sciences` remains `stillResearching: true` (1 entry). `/scholarships` redirects rather than 404s. Nav says "Opportunities," not "Money" — direct user correction, do not revert.
...
1. **`social-sciences` is the one remaining thin major-opportunity family** (`src/data/major-opportunities.ts`, 1 entry vs. 2–5 elsewhere). Needs real web research under the same rules as every other entry (verify on the official site, cost-first, never pay-to-attend).
2. **Design has never been explicitly signed off as final** — nothing's been rejected in a while, but confirm before treating the current system as settled.
3. **The paid provider upgrade (Claude Sonnet) is a deliberately deferred, not-forgotten decision.** The user explicitly redirected an attempt to wire this in mid-session ("no do provider setup for multiple ai models with openrouter it must all be free for now i can upgrade later"). `ANTHROPIC_API_KEY` stays reserved and unused until the user asks for it — OpenRouter's free rotation is the intended provider for the resume-rewrite call in the meantime, and that's a working state, not a gap.
4. **26 React Compiler lint warnings remain, by design** — all inside verified WebGL/animation code where the flagged imperative patterns are correct; reasoning is in `eslint.config.mjs`. Recorded debt, not urgent.
5. The `workflow`-scope PAT limitation is unresolved but no longer blocking anything (CI landed via the web-editor workaround) — fixing it (`git credential-osxkeychain erase` + a rescoped token) is a nice-to-have, not a task.

No §16K item, no known-open-item from earlier handoffs, and no "next feature" from the user's backlog remains outstanding as of this rewrite.
