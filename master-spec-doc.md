...
- **`/opportunities`** (§16R, and the social-sciences close-out later the same day) — a real unified directory, not scholarships-only. `lib/opportunities.ts` merges `data/scholarships.ts` (**14 entries**, both Golden Door and Ron Brown resolved/listed as of §16U) with `data/major-opportunities.ts` (**23 entries** across 8 families, tagged `kind`). **All 8 major families are now fully researched** — `social-sciences` was the last, closed with TASS, National Economics Challenge, Congressional Award, and We the People (see the Decisions Log entry immediately above this section). `/scholarships` redirects rather than 404s. Nav says "Opportunities," not "Money" — direct user correction, do not revert.
...
1. **Design has never been explicitly signed off as final** — nothing's been rejected in a while, but confirm before treating the current system as settled.
2. **The paid provider upgrade (Claude Sonnet) is a deliberately deferred, not-forgotten decision.** The user explicitly redirected an attempt to wire this in mid-session ("no do provider setup for multiple ai models with openrouter it must all be free for now i can upgrade later"). `ANTHROPIC_API_KEY` stays reserved and unused until the user asks for it — OpenRouter's free rotation is the intended provider for the resume-rewrite call in the meantime, and that's a working state, not a gap. `OPENROUTER_API_KEY` itself still needs adding to `app/.env.local` (local) and the Vercel dashboard (production) before OpenRouter activates at all — neither is something a session can do unattended.
